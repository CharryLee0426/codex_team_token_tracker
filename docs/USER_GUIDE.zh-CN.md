...
- **时间范围按钮**（Today · 7d · 30d · 90d · 1y）、**自团队套餐开始**（2026-08-25 00:00 PDT 起的全部用量）与 **自定义**（任意起止日期，最长一年）用于选择用量统计范围。**活跃时段**在选择今天或不超过 3 天的自定义范围时，为每个所选日期显示小时曲线；较长范围使用星期/小时热力图，悬停、键盘聚焦或点按单元格可查看具体日期及 token 数。**星期对比**显示所选结束日期所在的本地周一至周日，包含范围之外的同周日期，卡片会标明该周日期。贡献热力图仍保留较长的日历历史。
- **分享用量**（个人与团队页）—— 像移动端应用一样，把所选范围生成一张图片卡片：总 token 数、每日趋势条、API 等价费用、请求数、缓存命中率、输入/输出、设备数或活跃成员数、模型数，并带有你的名字和头像（个人）或组织的名称和头像（团队）。可复制到剪贴板、下载 PNG，或在浏览器支持时调用系统分享面板。卡片跟随仪表盘语言；不会包含邮箱、设备、会话或提示词，也不会上传任何内容 —— 图片在你的浏览器中本地生成。
