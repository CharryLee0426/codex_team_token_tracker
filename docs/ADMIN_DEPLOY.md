...
- **Pricing** is computed by the backend, never on devices (wire version 3, tracker 0.5.0). Clients upload token counts — plus the share of each bucket that ran above the 272K long-context threshold and a per-model breakdown of each session — and `packages/backend/convex/pricing.ts` prices them (USD per 1M tokens, cache-write rates and long-context tiers included). Older clients still send a device-computed `cost`; it is ignored, and their rows are billed at the standard tier until that device updates and re-syncs. The price table is read from <https://developers.openai.com/api/docs/pricing> by an hourly cron (`refresh OpenAI pricing`); a new `pricingSnapshots` row is written only when a rate changed, and every stored row is then re-priced automatically. Settings → Pricing on the dashboard shows the table in force, when it was last checked and any fetch error (the previous table stays in force on failure). Before the first successful read the seed table in `packages/shared/src/pricing.ts` applies; models the page does not list keep their seed rate, and unknown models fall back to their family and are flagged *est.* Admin commands, run from `apps/dashboard` (add `--prod` for production):
  - `npx convex run pricing:refresh` — read the page now instead of waiting for the cron.
  - `npx convex run pricing:setOverride '{"model":"gpt-5.7-nova","input":1.75,"cachedInput":0.175,"output":14}'` — pin a rate (also for a model the page does not list); `cacheWrite` and `long` are optional. `pricing:clearOverride '{"model":"…"}'` removes it. Both re-price history.
  - `npx convex run pricing:repriceAll '{}'` — re-run the sweep by hand; it is idempotent.
  Only OpenAI models with exact Codex-OAuth attribution are counted — API-key and non-OpenAI usage is dropped on the device.
