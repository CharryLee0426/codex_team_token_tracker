...
- **Pricing moved to the backend (wire version 3).** The tracker no longer computes the dollars it
  uploads. Buckets and sessions now carry token counts only — plus the share of each that came from
...
- **Local figures use the backend's table.** The tray, popover and CLI download the price table the
  ...
  the table was downloaded and how current OpenAI's list is. The bundled table remains the fallback
  before the first download.

### Removed

- **`~/.codex-tracker/pricing.json` overrides.** A local override could no longer affect what the
  dashboard shows, so it is gone; dashboard admins pin or correct a rate on the backend once, for every
  device (`npx convex run pricing:setOverride …`, see the admin guide).
